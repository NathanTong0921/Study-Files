...
class LeafDataset(Dataset[tuple[torch.Tensor, int]]):
    ...
    def __len__(self) -> int:
        """Return the number of examples in the manifest."""
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, int]:
        ...
        class_index = int(row.loc[list(CLASSES)].to_numpy().argmax())
...
def build_bonus_transforms(image_size: int) -> Callable[[Image.Image], torch.Tensor]:
    """Return label-preserving geometric augmentation for the optional bonus run."""
    return transforms.Compose(
        [
            transforms.Resize((image_size, image_size)),
